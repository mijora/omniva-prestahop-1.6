...
class AdminOmnivaOrdersController extends ModuleAdminController
{
	...
	public function initContent()
	{
        ...
        $this->context->smarty->assign(array(
			...
			'ajax' => Context::getContext()->shop->getBaseURL(true),			
	...
	public function displayAjax() 
	{
		...
		if($date != null AND $date != 'undefined' AND $date != '')
			$where  .= ' AND (oc.date_add LIKE "%'.$date.'%" OR a.date_upd LIKE "%'.$date.'%" 
		) ';

		$where2  = ' AND (a.date_add LIKE "%'.$date.'%" OR a.date_upd LIKE "%'.$date.'%" 
		OR k.date_upd LIKE "%'.$date.'%" OR k.date_add LIKE "%'.$date.'%"
		OR oc.date_add LIKE "%'.$date.'%") ';
